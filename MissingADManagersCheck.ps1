...
#Written by Mike McGlothern 8/28/2017, version 1.1
...
$STATUS = Get-ADUser -LDAPFilter "(&(!manager=*)(!useraccountcontrol:1.2.840.113556.1.4.803:=2)(Sn=*))" -SearchBase $SearchBase -Properties * | Select DisplayName, DistinguishedName
...
#Second 'statusformatted' variable used here, if Status is converted to HTML it will be non-null even though it has no data, breaking checks later on.
$STATUSFORMATTED = $Status | ConvertTo-HTML | Out-String
if ($STATUS) {
...
If ($STATUS) 
{
 Write-Verbose  "Sending email"
    $MessageBody += "The following users in OU $SearchScope do not have the 'Manager' field populated. This can impact other automation that relies on this value, Team Calendar views in Outlook, Delve, etc. and should be fixed. `r`n `r`n $STATUSFORMATTED"
